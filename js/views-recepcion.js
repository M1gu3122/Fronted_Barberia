...
(function () {
  ...
  function rDashboardRecepcion() {
    ...
    /* Citas asignadas hoy */
    html += '<section class="card"><div class="card-header"><div><div class="card-title">Citas de hoy</div><div class="card-sub">Atencion del dia</div></div>' +
    ...
    /* Proximas citas */
    html += '<section class="card"><div class="card-header"><div><div class="card-title">Proximas citas</div><div class="card-sub">Los proximos 3 dias</div></div></div>';
    ...
    html += '<section class="card"><div class="card-header"><div><div class="card-title">Horarios disponibles hoy</div><div class="card-sub">Por barbero</div></div>' +
      '<button class="btn btn-sm btn-ghost" style="margin-left:auto;" onclick="App.navigate(\'horarios\')">Ver todos</button></div>';
    html += '<div style="padding:12px;display:grid;gap:8px;">';
    d.barberos.filter(function (b) { return b.activo; }).slice(0, 4).forEach(function (b) {
      var libres = d.horariosLibres(b.id, d.iso(0)).filter(function (s) { return s.libre; });
      html += '<div style="display:flex;align-items:center;gap:10px;padding:10px;background:var(--sand);border-radius:9px;">' +
        UI.avatar(b.nombre, "avatar-sm") +
        '<div style="flex:1;"><span style="font-weight:600;font-size:13.5px;">' + b.nombre + "</span></div>" +
        '<span class="badge badge-completada badge-dotless">' + libres.length + " libres</span></div>";
    });
    html += "</div></section>";
  ...
  function rPerfilRecepcion() {
    ...
    html += '<section class="card"><div class="card-header"><div><div class="card-title">Datos de la cuenta</div></div></div>' +
      ...
      '<div class="field"><label class="field-label">Apellido</label><input class="input" value="Cardenas"></div></div>' +
      '<div class="field"><label class="field-label">Correo</label><input class="input" type="email" value="recepcion@corteperfecto.com"></div>' +
      '<div class="field"><label class="field-label">Telefono</label><input class="input" value="301 222 3344"></div></div>' +
