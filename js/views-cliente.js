...
(function () {
  ...
  function rDashboard() {
    ...
    }).sort(function (a, b) { return (a.fecha + a.hora) < (b.fecha + b.hora) ? -1 : 1; })[0];
    ...
    /* --- Hero proxima cita --- */
    if (proxima) {
      var s = d.servicio(proxima.servicio);
      var b = d.barbero(proxima.barbero);
      html +=
    ...
    (futuras.length ? futuras.map(function (c) {
      ...
      return '<div class="appt-tile ' + c.estado + '">' +
        ...
        '<div>' + UI.estadoBadge(c.estado) + "</div></div>";
    }).join("") :
      '<div class="empty" style="padding:30px;"><div class="empty-text" style="margin:0;">Aun no tienes citas programadas.</div></div>') +
    "</div></section>";
  ...
  function rReservar() {
    ...
    '<div class="steps">' + pasos.map(function (p, i) {
      var n = i + 1;
      var cls = n === reserva.paso ? "active" : (n < reserva.paso ? "done" : "");
      return '<div class="step ' + cls + '"><span class="step-dot">' + (n < reserva.paso ? '\u2713' : n) + "</span>" +
        '<span class="step-label">' + p + "</span></div>" +
        (n < pasos.length ? '<div class="step-line"></div>' : "");
    }).join("") + "</div></section>";

    html += '<div id="reserva-panel" style="margin-top:16px;">';
    if (reserva.paso === 1) html += pasoServicio();
    else if (reserva.paso === 2) html += pasoBarbero();
    else if (reserva.paso === 3) html += pasoFecha();
    else if (reserva.paso === 4) html += pasoHora();
    else html += pasoConfirmacion();
    html += "</div>";
  ...
  function pasoServicio() {
    var html = '<section class="card"><div class="card-header"><div><div class="card-title">Selecciona el servicio</div><div class="card-sub">Elige el estilo que quieres</div></div></div>' +
      '<div class="card-body"><div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:12px;">';
    DB.servicios.filter(function (s) { return s.activo; }).forEach(function (s) {
      var sel = reserva.servicio === s.id ? " selected" : "";
      html += '<div class="service-card' + sel + '" data-sel="servicio" data-id="' + s.id + '">' +
        ...
        '<span style="font-weight:700;color:var(--brass-dim);margin-left:auto;">' + DB.formatPrecio(s.precio) + "</span></div>" +
        "</div>";
    });
    html += "</div></div>";
    html += '<div class="card-footer" style="display:flex;justify-content:flex-end;gap:10px;">' +
      '<button class="btn btn-primary" id="reserva-next" ' + (reserva.servicio ? "" : "disabled") + '>Continuar <i class="fas fa-arrow-right"></i></button></div></section>';
    return html;
  }

  function pasoBarbero() {
    var html = '<section class="card"><div class="card-header"><div><div class="card-title">Elige tu barbero</div><div class="card-sub">Todos son profesionales certificados</div></div></div>' +
      '<div class="card-body"><div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:12px;">';
    DB.barberos.filter(function (b) { return b.activo; }).forEach(function (b) {
      var sel = reserva.barbero === b.id ? " selected" : "";
      html += '<div class="barber-card' + sel + '" data-sel="barbero" data-id="' + b.id + '">' +
        ...
        '<div style="font-size:12px;color:var(--st-completada);"><i class="fas fa-circle-check"></i> Disponible hoy</div></div></div>';
    });
    html += "</div></div>";
    html += '<div class="card-footer" style="display:flex;justify-content:space-between;gap:10px;">' +
      '<button class="btn btn-ghost" onclick="App.goReserva(1)"><i class="fas fa-arrow-left"></i> Atras</button>' +
      '<button class="btn btn-primary" id="reserva-next" ' + (reserva.barbero ? "" : "disabled") + '>Continuar <i class="fas fa-arrow-right"></i></button></div></section>';
    return html;
  }
  ...
  function pasoFecha() {
    ...
    var diaSemanaInicio = primero.getDay(); // 0=domingo
    var diasEnMes = new Date(ano, mes + 1, 0).getDate();
    var nomMes = primero.toLocaleDateString("es-CO", { month: "long", year: "numeric" });

    var html = '<section class="card"><div class="card-header"><div><div class="card-title">Selecciona la fecha</div><div class="card-sub" style="text-transform:capitalize;">' + nomMes + "</div></div></div>";
    html += '<div class="card-body">';
    html += '<div class="cal-head">' + ["Dom", "Lun", "Mar", "Mie", "Jue", "Vie", "Sab"].map(function (d) { return "<span>" + d + "</span>"; }).join("") + "</div>";
    html += '<div class="cal-grid">';
    for (var i = 0; i < diaSemanaInicio; i++) html += '<div class="cal-day other"></div>';
    for (var dia = 1; dia <= diasEnMes; dia++) {
      var d = new Date(ano, mes, dia);
      var fechaIso = d.toISOString().slice(0, 10);
      var cls = "cal-day";
      ...
      html += '<button type="button" class="' + cls + '" data-fecha="' + fechaIso + '"' + (d < today ? " disabled" : "") + ">" + dia + dots + "</button>";
    }
    html += "</div></div>";
    html += '<div class="card-footer" style="display:flex;justify-content:space-between;gap:10px;">' +
      '<button class="btn btn-ghost" onclick="App.goReserva(2)"><i class="fas fa-arrow-left"></i> Atras</button>' +
      '<button class="btn btn-primary" id="reserva-next" ' + (reserva.fecha ? "" : "disabled") + '>Continuar <i class="fas fa-arrow-right"></i></button></div></section>';
    return html;
  }

  function pasoHora() {
    var slots = DB.horariosLibres(reserva.barbero, reserva.fecha);
    var html = '<section class="card"><div class="card-header"><div><div class="card-title">Selecciona la hora</div><div class="card-sub">Solo se muestran horarios disponibles</div></div></div>' +
      '<div class="card-body">' + UI.timeGrid(slots, reserva.hora) +
      ...
      '<span><span class="badge badge-neutral badge-dotless">Ocupado</span></span></div></div>' +
      ...
      '<button class="btn btn-primary" id="reserva-next" ' + (reserva.hora ? "" : "disabled") + '>Continuar <i class="fas fa-arrow-right"></i></button></div></section>';
    return html;
  }

  function pasoConfirmacion() {
    var s = DB.servicio(reserva.servicio), b = DB.barbero(reserva.barbero);
    var html = '<section class="card"><div class="card-header"><div><div class="card-title">Confirma tu cita</div><div class="card-sub">Revisa que todo este en orden</div></div></div>';
    html += '<div class="card-body">';
    ...
    html += [
      ...
      return '<div style="display:flex;align-items:center;gap:12px;padding:10px 12px;background:var(--sand);border-radius:9px;">' +
        ...
        '<div style="font-weight:600;font-size:14px;">' + f[1] + "</div></div></div>";
    }).join("");
    html += "</div></div>";
    html += '<div class="card-footer" style="display:flex;justify-content:space-between;gap:10px;">' +
      '<button class="btn btn-ghost" onclick="App.goReserva(4)"><i class="fas fa-arrow-left"></i> Atras</button>' +
      '<button class="btn btn-primary" id="reserva-confirmar"><i class="fas fa-circle-check"></i> Confirmar cita</button></div></section>';
    return html;
  }
  ...
  function rMisCitas() {
    ...
    function tile(c) {
      ...
      return '<div class="appt-tile ' + c.estado + '">' +
        ...
        '<div style="display:flex;align-items:center;gap:10px;">' + UI.estadoBadge(c.estado) + acciones + "</div></div>";
    }

    var html = "";
    html += '<section class="card"><div class="card-header"><div><div class="card-title">Proximas citas</div><div class="card-sub">' + activas.length + " programadas</div></div></div>";
    ...
    html += '<section class="card" style="margin-top:16px;"><div class="card-header"><div><div class="card-title">Citas anteriores</div><div class="card-sub">' + pasadas.length + " registros</div></div></div>";
    html += '<div style="padding:14px;display:grid;gap:8px;">' +
    ...
    region.addEventListener("click", function (e) {
      ...
      if (accion === "cancelar") {
        UI.confirm({
          ...
          onConfirm: function () {
            DB.actualizarEstadoCita(citaId, "cancelada");
  ...
  function rReprogramar() {
    ...
    var html = '<section class="card"><div class="card-header"><div><div class="card-title">Reprogramar cita</div><div class="card-sub">Selecciona nueva fecha y hora</div></div></div>';
    html += '<div class="card-body">';

    /* Cita actual */
    html += '<div style="display:grid;gap:10px;margin-bottom:20px;">';
    html += '<div class="appt-tile ' + orig.estado + '">' +
      ...
      '<div>' + UI.estadoBadge(orig.estado) + "</div></div>";
    html += "</div>";

    /* Nueva fecha */
    html += '<div class="field"><label class="field-label">Nueva fecha <span class="req">*</span></label>';
    html += '<input type="date" class="input" id="reprogramar-fecha" value="' + fechaSel + '" min="' + DB.iso(1) + '"></div>';

    /* Horas */
    html += '<div class="field" style="margin-top:16px;"><label class="field-label">Nuevos horarios disponibles <span class="req">*</span></label>';
    html += '<div id="reprogramar-horas">' + UI.timeGrid(slots, horaSel) + "</div></div>";

    html += "</div>";
    html += '<div class="card-footer" style="display:flex;justify-content:space-between;gap:10px;">' +
      '<button class="btn btn-ghost" onclick="App.navigate(\'mis-citas\')">Cancelar</button>' +
      '<button class="btn btn-primary" id="reprogramar-confirmar" ' + (horaSel ? "" : "disabled") + '><i class="fas fa-circle-check"></i> Confirmar cambio</button></div></section>';
    return html;
  }
  ...
  function rHistorial() {
    var d = DB;
    var historial = d.citas.filter(function (c) { return c.cliente === 1 && c.estado === "completada"; })
      .sort(function (a, b) { return a.fecha < b.fecha ? 1 : -1; });

    var html = '<section class="card"><div class="card-header"><div><div class="card-title">Historial de servicios</div><div class="card-sub">' + historial.length + " servicios realizados</div></div></div>";
    html += '<div class="table-wrap"><table class="table table-responsive">';
    html += "<thead><tr>" +
      ["Fecha", "Servicio", "Barbero", "Duracion", "Estado"].map(function (h) { return "<th>" + h + "</th>"; }).join("") +
      "</tr></thead><tbody>";
    if (!historial.length) {
      html += '<tr><td colspan="5"><div class="empty"><div class="empty-ico"><i class="fas fa-clock-rotate-left"></i></div><div class="empty-title">Sin historial aun</div><div class="empty-text">Cuando completes tu primer servicio aparecera aqui.</div></div></td></tr>';
    }
  ...
  function rNotificaciones() {
    ...
    var html = '<section class="card"><div class="card-header"><div><div class="card-title">Notificaciones</div><div class="card-sub">Novedades de tus citas</div></div>' +
    ...
    DB.notificaciones.forEach(function (n) {
      var t = tipos[n.tipo] || ["cita", "fa-bell"];
      html += '<div class="notif' + (n.leida ? "" : " unread") + '" data-notif="' + n.id + '">' +
  ...
  function rPerfilCliente() {
    ...
    html += '<section class="card"><div class="card-body" style="text-align:center;">' +
      ...
      '<button class="btn btn-sm btn-ghost"><i class="fas fa-camera"></i> Cambiar foto</button></div>' +
    ...
    /* Formulario datos */
    html += '<section class="card"><div class="card-header"><div><div class="card-title">Datos personales</div><div class="card-sub">Informacion de tu cuenta</div></div></div>';
    html += '<div class="card-body" style="display:grid;gap:14px;">' +
    ...
    html += '<div class="card-footer" style="display:flex;justify-content:flex-end;gap:10px;">' +
      ...
      '<button class="btn btn-primary" id="guardar-perfil"><i class="fas fa-floppy-disk"></i> Guardar cambios</button></div></section>';

    /* Cambiar contrasena */
    html += '<section class="card" style="grid-column:1/-1;"><div class="card-header"><div><div class="card-title">Cambiar contrasena</div><div class="card-sub">Actualiza el acceso a tu cuenta</div></div></div>';
    html += '<div class="card-body" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px;">' +
      ...
      "</div>";
    html += '<div class="card-footer" style="display:flex;justify-content:flex-end;"><button class="btn btn-primary" id="guardar-pass"><i class="fas fa-key"></i> Actualizar contrasena</button></div></section>';
