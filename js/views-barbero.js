...
(function () {
  ...
  function tileCita(c, acciones) {
    ...
    return '<div class="appt-tile ' + c.estado + '">' +
      ...
      '<div style="display:flex;align-items:center;gap:8px;">' + UI.estadoBadge(c.estado) + (acciones || "") + "</div></div>";
  }
  ...
  function rDashboard() {
    ...
    /* Saludo */
    html += '<section class="card hero-cita">' +
      ...
      '<div style="color:var(--brass-light);font-size:14px;font-weight:600;">' + proxima.hora + " hs</div></div>" : "") +
      "</div></section>";
    ...
    html += '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:14px;">' +
      [[hoy.length, "Citas hoy", "fa-calendar-day", "var(--st-atencion-bg)", "var(--st-atencion)"],
       ...
       [completadas, "Completadas", "fa-circle-check", "var(--st-completada-bg)", "var(--st-completada)"]].map(function (k) {
    ...
    /* Agenda del dia */
    html += '<section class="card"><div class="card-header"><div><div class="card-title">Agenda de hoy</div><div class="card-sub">' + DB.formatFechaLarga(DB.iso(0)) + "</div></div>" +
  ...
  function rAgenda() {
    var html = '<section class="card"><div class="card-header">' +
      ...
      "</div></div>";
    html += '<div class="card-body" style="padding-top:16px;">';
    if (agendaModo === "dia") html += agendaDia();
    else html += agendaSemana();
    html += "</div></section>";
    return html;
  }
  ...
  function agendaDia() {
    ...
    for (var h = 9; h <= 18; h++) {
      ...
      html += '<div class="agenda-hour">' + hh + "</div>";
      var delSlot = citas.filter(function (c) { return c.hora.slice(0, 2) === String(h).padStart(2, "0"); });
      if (delSlot.length) {
        delSlot.forEach(function (c) {
          var cl = DB.cliente(c.cliente), s = DB.servicio(c.servicio);
          html += '<div class="agenda-item">' + tileCita(c, '<button class="btn btn-sm btn-ghost" data-ver="' + c.id + '"><i class="fas fa-eye"></i></button>') + "</div>";
        });
      } else {
        html += '<div class="agenda-item"><div style="padding:12px 14px;border-radius:10px;border:1px dashed var(--line);color:var(--mist);font-size:12.5px;">Horario disponible</div></div>';
      }
    }
    html += "</div>";
    return html;
  ...
  function agendaSemana() {
    ...
    var html = '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:10px;">';
    dias.forEach(function (dia) {
      var esHoy = dia.fecha === DB.iso(0);
      html += '<div class="card" style="' + (esHoy ? "border-color:var(--brass);" : "") + '"><div class="card-header" style="padding:10px 12px;">' +
        '<div class="card-title" style="font-size:13px;text-transform:capitalize;">' + dia.nombre + "</div></div>" +
  ...
  function rDetalleCita(citaId) {
    ...
    if (c.estado === "pendiente" || c.estado === "confirmada") {
      acciones =
      '<button class="btn btn-primary" data-estado="espera"><i class="fas fa-user-clock"></i> Marcar en espera</button>';
    } else if (c.estado === "espera") {
      acciones =
      '<button class="btn btn-primary" data-estado="atencion"><i class="fas fa-scissors"></i> Marcar en atencion</button>';
    ...
    var html = '<section class="card"><div class="card-header"><div><div class="card-title">Detalle de cita #' + c.id + '</div><div class="card-sub">' + DB.formatFechaLargaConAno(c.fecha) + ' · ' + c.hora + ' hs</div></div>' +
    ...
    html += '<div class="card-body" style="display:grid;gap:14px;">' +
      ...
      '<div class="cell-muted">' + cl.telefono + " · " + cl.correo + "</div></div></div>";
    ...
    html += '<div class="card-footer" style="display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap;">' +
      '<button class="btn btn-ghost" onclick="App.navigate(\'dashboard\')"><i class="fas fa-arrow-left"></i> Volver</button>' +
      '<div style="display:flex;gap:8px;">' + acciones + "</div></div></section>";
    return html;
  }
  ...
  function rMisCitasBarbero() {
    var html = '<section class="card"><div class="card-header"><div><div class="card-title">Mis citas</div><div class="card-sub">Proximas y pendientes</div></div></div>';
    html += '<div style="padding:14px;display:grid;gap:8px;">';
    var futuras = citasFuturas();
    if (!futuras.length) {
      html += '<div class="empty" style="padding:34px;"><div class="empty-ico"><i class="fas fa-calendar-check"></i></div><div class="empty-title">Sin citas proximas</div><div class="empty-text">Cuando te asignen citas apareceran aqui.</div></div>';
    }
    futuras.forEach(function (c) {
      ...
      html += '<div class="appt-tile ' + c.estado + '">' +
        ...
        '<button class="btn btn-sm btn-ghost" data-ver="' + c.id + '"><i class="fas fa-eye"></i></button></div></div>';
    });
    html += "</div></section>";
    return html;
  }
  ...
  function rHistorialBarbero() {
    ...
    var html = '<section class="card"><div class="card-header"><div><div class="card-title">Citas atendidas</div><div class="card-sub">' + hist.length + " servicios completados</div></div></div>";
    html += '<div class="table-wrap"><table class="table table-responsive"><thead><tr>' +
      ...
      "</tr></thead><tbody>";
    if (!hist.length) {
      html += '<tr><td colspan="5"><div class="empty"><div class="empty-ico"><i class="fas fa-clock-rotate-left"></i></div><div class="empty-title">Sin historial</div><div class="empty-text">Aun no has atendido servicios.</div></div></td></tr>';
    }
    hist.forEach(function (c) {
      ...
      html += "<tr>" +
        ...
        '<td data-label="Estado">' + UI.estadoBadge(c.estado) + "</td></tr>";
    });
  ...
  function rPerfilBarbero() {
    ...
    html += '<section class="card"><div class="card-header"><div><div class="card-title">Datos profesionales</div><div class="card-sub">Tu informacion de trabajo</div></div></div>';
    html += '<div class="card-body" style="display:grid;gap:14px;">' +
      ...
      "</div></div>";
    html += '<div class="card-footer" style="display:flex;justify-content:flex-end;"><button class="btn btn-primary" id="guardar-perfil"><i class="fas fa-floppy-disk"></i> Guardar cambios</button></div></section>';
    html += "</div>";
    return html;
  }
