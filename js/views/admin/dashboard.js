...
var today = DB.iso(0);
      ...
      // Cargar proximasCitas de forma segura y aislada
      _dashboardData.proximasCitas = [];
      try {
        var temp = await api.getProximasCitas();
        
        // Validar estructura esperada: { citas: [...] } o array directo
        if (temp && typeof temp === 'object') {
          var citas = temp.citas || temp;
          if (Array.isArray(citas)) {
            _dashboardData.proximasCitas = citas;
          }
        }
      } catch (e) {
        // Errores en la llamada se ignoran - no rompen el dashboard
      }
      
      // Transformar datos para el HTML - SIEMPRE generar array transformado
      // Esto asegura que incluso si hay 0 datos, el HTML no rompa
      _dashboardData.proximasCitasDisplay = (_dashboardData.proximasCitas || []).map(function (c) {
        try {
          var hora = c.hora || DateUtils.timeFromDateTime(c.fecha_hora); // la API devuelve fecha y hora por separado
          var estado = String(c.estado || c.estado_cita || "").toLowerCase();
          var fecha = c.fecha ? DateUtils.formatDate(c.fecha) : DateUtils.dateFromDateTime(c.fecha_hora);
          return {
            hora: hora,
            estado: estado,
            fecha: fecha,
            nombres: c.cliente?.nombre || "Cliente",
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(340px,1fr));gap:16px;">
        ...
        <section class="card">
          ...
          <div style="padding:12px;display:grid;gap:8px;">
            ${_dashboardData.proximasCitasDisplay.slice(0, 5).map(function (c) {
              return `
                <div class="appt-tile ${c.estado}">
                  <div class="appt-time">${c.fecha ? c.fecha + " " : ""}${c.hora}</div>
                  <div class="appt-main">
                    <div class="appt-title">${c.nombres || "Cliente"}</div>
                    <div class="appt-sub">${c.servicios?.[0]?.nombre || "Servicio"} · ${c.barbero?.nombre || "Barbero"}</div>
                  </div>
              ...
              return `
                <div style="margin-bottom:14px;">
                  ...
                  <div style="height:8px;background:var(--bone);border-radius:999px;overflow:hidden;">
                    <div class="barra" data-w="${pct}" style="height:100%;width:${pct}%;background:linear-gradient(90deg,var(--brass-light),var(--brass));border-radius:999px;"></div>
                  </div>
                </div>`;
        ...
        <section class="card">
          ...
          <div class="card-body" style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
            <div class="kpi" style="background:var(--st-completada-bg);border:1px solid #c8e4ca;">
              ...
              <div style="font-size:12px;color:var(--smoke);margin-top:8px;">${icActual.citas_completadas || 0} citas · Ticket: ${icActual.ticket_promedio ? d.formatPrecio(icActual.ticket_promedio) : "$0"}</div>
            </div>
            <div class="kpi" style="background:var(--st-atencion-bg);border:1px solid #f4d9b4;">
              <div class="kpi-top"><span class="kpi-ico" style="background:var(--st-atencion);"><i class="fas fa-chart-line"></i></span><span class="kpi-label">Variación</span></div>
              ...
              <div style="font-size:12px;color:var(--smoke);margin-top:8px;">Citas: ${icVar.citas_pct || 0}% · Ticket: ${icVar.ticket_pct || 0}%</div>
            </div>
          </div>
        </section>
        ...
        <section class="card">
          ...
          <div class="card-body">
            <div style="overflow-x:auto;">
              <table class="table">
                <thead><tr><th>Barbero</th><th>Citas</th><th>Completadas</th><th>Canceladas</th><th>Ingresos</th><th>Ocupación</th></tr></thead>
                <tbody>
                  ${_dashboardData.barberosRendimiento.map(function (b) {
                    return `<tr>
                      ...
                      <td data-label="Ocupación"><div class="barra" data-w="${Math.min(b.ocupacion_promedio || 0, 100)}" style="height:8px;width:${Math.min(b.ocupacion_promedio || 0, 100)}%;background:linear-gradient(90deg,var(--brass-light),var(--brass));border-radius:999px;"></div><span style="font-size:11px;color:var(--smoke);">${b.ocupacion_promedio || 0}%</span></td>
                    </tr>`;
          ...
          <div class="card-body" style="display:flex; justify-content:center;">
            <div style="display:flex;gap:16px;flex-wrap:wrap;margin-bottom:16px;">
              <div class="kpi" style="background:var(--st-cancelada-bg);border:1px solid #f0cdc9;">
                <div class="kpi-top"><span class="kpi-ico" style="background:var(--st-cancelada);"><i class="fas fa-ban"></i></span><span class="kpi-label">Tasa cancelación</span></div>
  ...
  function bindBarras() {
    ...
    setTimeout(function () {
      ...
      // Barras de top servicios y ocupacion
      ...
      // Donut: escala suave
      var donut = region.querySelector(".donut");
      if (donut) {
        gsap.fromTo(donut,
          { scale: 0.8, opacity: 0 },
      ...
      // Tarjetas KPI: entrada escalonada
      gsap.fromTo(region.querySelectorAll(".card.kpi"),
        { y: 14, opacity: 0 },
        { y: 0, opacity: 1, duration: 0.5, ease: "power2.out", stagger: 0.05 });
    }, 60);
  }

  // Registro de vista con carga asíncrona
